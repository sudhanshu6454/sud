...
<section class="archive-head rule">
	<span class="kicker kicker--grey"><a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'Home', 'filmybuff' ); ?></a> / <?php echo is_category() ? esc_html__( 'Section', 'filmybuff' ) : esc_html__( 'Archive', 'filmybuff' ); ?></span>
	<div class="row">
...
<?php if ( is_category() ) : ?>
<nav class="cat-tabs" aria-label="<?php esc_attr_e( 'Sections', 'filmybuff' ); ?>" style="padding-top:20px">
...
<aside class="sidebar"><?php fb_trending( is_category() ? sprintf( __( 'Most read in %s', 'filmybuff' ), $term->name ) : null, is_category() ? $term->term_id : 0 ); fb_follow_box(); dynamic_sidebar( 'sidebar-1' ); ?></aside>
</div></section>
</div></main>
<?php get_footer();
