...
?>
<?php /* With no brand battle to show, the lab takes the full width instead of leaving half the row
	   empty under a "nothing running" line. */ ?>
<section class="mm-section"<?php echo $mm_battles ? ' style="display:grid;grid-template-columns:1fr 1fr"' : ''; ?>>
	<div style="padding:56px 44px 56px var(--gutter);<?php echo $mm_battles ? 'border-right:var(--rule);' : ''; ?>display:flex;flex-direction:column;gap:28px;background:var(--mm-bone)">
	...
	<div style="padding:56px var(--gutter);display:flex;flex-direction:column;gap:28px">
		...
		<?php $mm_battle = $mm_battles[0]; $mm_sides = array_map( 'trim', explode( ' vs ', str_ireplace( ' vs. ', ' vs ', $mm_battle->post_title ) ) ); ?>
