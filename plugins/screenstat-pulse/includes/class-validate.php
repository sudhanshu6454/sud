...
final class SSPulse_Validate {
	...
	public static function signals( $s, bool $partial = false ): ?string {
		...
		$known = array_merge( array_keys( SSPulse_Model::RANGES ), array( 'release', 'holiday', 'comp' ) );
		foreach ( $s as $k => $v ) {
			...
			if ( $k === 'holiday' ) { if ( ! in_array( (string) $v, self::HOLIDAY, true ) ) { return "holiday must be '0' or '1'"; } continue; }
			if ( $k === 'comp' ) { if ( ! in_array( $v, self::COMP, true ) ) { return 'comp must be none, moderate or heavy'; } continue; }
